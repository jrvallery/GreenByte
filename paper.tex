...
\begin{itemize}
  ...
  \centering                                                                                                                      
      ...
      \multicolumn{4}{l}{\textit{Calibrated control parameters}} \\
        Day heating setpoint   & $T_{\text{sp,day}}$   & $19.17$°C (66.5°F)  & Dominant calibration lever; GreenLight default 14.4°C (58°F) \\
        Night heating setpoint & $T_{\text{sp,night}}$ & $17.17$°C (62.9°F)  & \\
        Heater power (stage 1) & $P_{\text{blow}}$     & $1{,}500$ W          & Electric resistance; default 23,478 W fires 14$\times$ oversized burst \\
        Fan dead zone          & $\Delta T_{\text{dead}}$ & 5°C (winter/spring); 8°C (shoulder) & Window-specific; controls solar overshoot \\
        \hline
      \multicolumn{4}{l}{\textit{Envelope parameters (at physical defaults)}} \\
        Infiltration leakage & $c_{\text{leakage}}$ & $3.0 \times 10^{-4}$ & Default; tuning yields ${<}1\%$ MAE improvement \\
        Cover conductivity   & $\lambda_{\text{Rf}}$ & $0.021$ W/(m$\cdot$K) & Default; envelope search flat once control params correct \\
        \hline                                                                                                                           
    ...
    \item \textbf{Model Architecture:} We compared 13 model families on the same 500-sample dataset using 5-fold cross-validation R² averaged across all three targets (Table~\ref{tab:model-comparison}). Gradient boosting methods (XGBoost, LightGBM, sklearn GradientBoosting) consistently outperformed the MLP by approximately 0.06 R², with the largest gap on cost prediction (0.88 vs.\ 0.78 CV R²) — the primary optimization target for the Iris planner. We therefore replaced the MLP surrogate with an XGBoost ensemble (300 trees per target via \texttt{MultiOutputRegressor}) wrapped in a \texttt{StandardScaler} pipeline. The dataset was split 85/15 train/test. Feature engineering converts the raw month integer to cyclical $(\sin, \cos)$ encoding so the December--January boundary is smooth in feature space.

\begin{table}[h]
  \centering
  \caption{Model comparison — 5-fold CV R² (higher is better) and inference speed. All models trained on the same 500-sample LHS sweep with cyclical month encoding.}
  \label{tab:model-comparison}
  \begin{tabular}{lccccr}
    \hline
    \textbf{Model} & \textbf{Avg CV R²} & \textbf{Temp} & \textbf{RH} & \textbf{Cost} & \textbf{Speed (µs)} \\
    \hline
    GradientBoosting   & \textbf{0.906} & 0.941 & 0.894 & \textbf{0.884} & 5.4 \\
    LightGBM           & 0.901          & 0.938 & \textbf{0.905} & 0.860 & 3.4 \\
    \textbf{XGBoost (ours)} & 0.901    & 0.936 & 0.893 & 0.874 & \textbf{1.8} \\
    ExtraTrees         & 0.896          & 0.923 & 0.910 & 0.855 & 25.9 \\
    GaussianProcess    & 0.896          & \textbf{0.952} & 0.874 & 0.862 & 6.8 \\
    SVR (RBF)          & 0.863          & 0.931 & 0.832 & 0.825 & 28.6 \\
    MLP 128×64         & 0.845          & 0.933 & 0.818 & 0.784 & 0.5 \\
    Linear / Ridge     & 0.749          & 0.884 & 0.693 & 0.670 & 0.3 \\
    \hline
  \end{tabular}
\end{table} 
