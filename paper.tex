...
\begin{abstract}
  Managing microclimates in small-scale greenhouses is traditionally hindered by the high cost of sensor arrays and the long duration required for manual data collection. We present a methodology for creating a personalized greenhouse climate predictor using a "digital twin" approach. By calibrating the GreenLight simulator via Nelder-Mead optimization to match the physical thermal envelope of a specific residential greenhouse, we generated dataset spanning 500 diverse environmental scenarios. This data was used to train a Multi-output Feed Forward Neural Network (MLP surrogate) capable of predicting internal temperature, relative humidity, and operating costs based on external weather inputs. Our results demonstrate a high predictive accuracy with an average $R^2$ of 0.92 and a Mean Absolute Error (MAE) of 0.6°C for temperature and 0.06 for operating costs.
\end{abstract}
...
Quantitative Objectives: We hope to achieve a Root Mean Squared Error (RMSE) 2.0°C (3.6°F) for temperature predictions and a test error of at most 0.15 normalized MAE across all metrics (temperature, relative humidity, and heating energy consumption).
...
\begin{itemize}
  ...
  \centering                                                                                                                      
    ...
    \item \textbf{Model Architecture:} We implemented a feed forward Neural Network (MLP Surrogate) to map these simulated inputs to the desired output variables having hidden layers (128, 64, 32) with and the pipeline being Linear to LayerNorm to GELU to Dropout to Linear (3 outputs). Our csv data set was split into training, validation, and test datasets with the percentage being 80/20/20 respectively. We had it run for 1000 epochs with early stoppage implemented, stopping the training as soon as we see the model converge. Because of this on average the model would stop training after around 300-400 epochs. We also tuned our hyperparameters until we got a lower MAE on average across all three output variables leaving us with a learning rate of 1e-3, a weight decay 3e-4, a batch size of 32 all the while we use the SGD optimizer. 
    \item \textbf{Design Justification:} This architecture was chosen for a number of reasons. To start, GreenLight provided us with a way to create a large greenhouse dataset with many variables, both output and input. When we compare the number of variables in each entry of our dataset to the variables in public datasets like Kaggle, we have hundreds more to predict/edit to ensure we have an accurate model specifically tailored to our greenhouse. The specific Neural Network with MLP surrogate was chosen for its ability to handle the complexity our dataset has to offer, being able to find the underlying complexity between input and output variables better than linear regression algorithms like XGBoost. 
...
unknownskl. “GitHub - Unknownskl/Greenlight: Greenlight Is an Open-Source Client for XCloud and Xbox Home Streaming Made in Typescript.” GitHub, 11 Dec. 2025, github.com/unknownskl/greenlight. Accessed 24 Apr. 2026.
